...
import ModalImport from '@/components/ui/Modal';
import { GenericConfigPanel } from '@/components/config-controls/GenericConfigPanel';
import { FloatingActionBar } from '@/components/config-controls/FloatingActionBar';
...
export default function ToolPage({ params }: { params: Promise<{ toolKey: string }> }) {
  // Mock param resolution for preview safety
  const [resolvedParams, setResolvedParams] = useState<{ toolKey: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (params) {
      ...
      setResolvedParams({ toolKey: 'preview-tool-id' });
    }
  }, [params]);

  const router = useRouter();
  const toolKey = resolvedParams ? decodeURIComponent(resolvedParams.toolKey) : 'loading...';
  ...
  const { showToast, setTheme } = useToast();
    ...
    try {
      ...
      if (encoded) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          setTimeout(() => showToast('已加载分享的配置，沉浸式体验中...'), 500);
        }
      }
    } catch { }
  }, [tool]);
  ...
  return (
    <main className="relative flex h-screen w-full bg-black font-sans text-slate-100 overflow-hidden">
      ...
      {/* Toast notification now handled by global ToastProvider */}
